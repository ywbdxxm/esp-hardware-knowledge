...
本项目为半导体和电子元器件的本地权威 PDF 提供可追溯的资料层和确定性检索层，当前覆盖
ESP32-C3、ESP32-S3 与 BQ2407x。Docling 与 RapidOCR 在本地解析文档，SQLite FTS5 负责
中英文关键词检索；每个结果都保留精确器件身份、原 PDF、SHA-256 和物理页码。Markdown
只用于定位候选证据，原 PDF 始终是最终权威来源。
